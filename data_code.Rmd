...
```{r}
model1 <- polr(as.factor(indicator) ~ RD_intensity + patents_2018, data = final_filtered_data, Hess=TRUE)
...
```{r}
model2 <- polr(as.factor(indicator) ~ (RD_intensity)**2 + patents_2018, data = final_filtered_data, Hess=TRUE)

summary(model2)
```
## model3 => indicator = r&d intensity + (patent 2018)**2
```{r}
model3 <- polr(as.factor(indicator) ~ RD_intensity + (patents_2018)**2, data = final_filtered_data, Hess=TRUE)

summary(model3)
```
## model4 => indicator = r&d intensity * patent
```{r}
model4 <- polr(as.factor(indicator) ~ RD_intensity * patents_2018, data = final_filtered_data, Hess=TRUE)
...
```{r}
stargazer(final_filtered_data, type = "html")
```
